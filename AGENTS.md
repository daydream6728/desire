...
- `PRS/<repo>/<number>.md` is the standing note of one pull request, below
...
Every turn writes to MEMORY_REPO, whatever else it did: the `PRS/` file of every head it touched,
...
`PRS/<repo>/<number>.md` says what one pull request does, where it stands, what it waits on, and
carries one line per turn that touched it. **Every session rewrites the file of every head it
...
([#124](https://github.com/toumix/desire/issues/124)). One writer per file and one file per head,
written by the turn that did the work rather than by a nightly re-derivation.

`<repo>` is the repository name alone: `PRS/discopy/489.md`. **Its lifetime ends with the pull
request** — the turn that sees it merged or closed deletes the file, archiving nothing: what a
merged PR did is in `main`, what a closed one was is on GitHub. `sweep.py` reads the directory
against the live open-PR list, so a head with no file and a file with no head are both findings.

[`template/memory/PRS/TEMPLATE.md`](template/memory/PRS/TEMPLATE.md) is the shape. Every fact cheap enough to
re-read live — state, mergeable state, checks — carries the date it was read, so a stale line reads
as stale instead of as true.
...
## Detail                  links to `USER_TODO.md`, the board, the `PRS/` files that changed
...
Since the `PRS/` files carry what each head did, this section says what the *round* did.
