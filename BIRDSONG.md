...
- **it reads `PRS/<repo>/<number>.md` rather than re-deriving the queue.** Every session that
  ...
  list of open AGENT-owned PR numbers, which `sweep.py` checks against `PRS/` and reports as
