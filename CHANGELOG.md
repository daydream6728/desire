...
**Agent-authored posts are signed `Generated by Codex`** — `AGENT_FOOTER` changes from the
Claude URL token to the displayed signature. `AGENT_FOOTER_ALIASES` retains that token for
