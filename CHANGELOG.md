...
**Agent-authored posts are signed `Generated by Codex`**
([#122](https://github.com/toumix/desire/pull/122), closes
[#121](https://github.com/toumix/desire/issues/121)) — `AGENT_FOOTER` changes from the Claude
URL token to the displayed signature. `AGENT_FOOTER_ALIASES` retains that token for
historical detection only, so old adopted-PR replies stay answered. A new post uses the plain
footer or links that exact label to a copied and reviewed HTTPS Codex snapshot; internal thread
IDs never become links. `sweep.py` replaces its arbitrary last-line substring check with the
exact plain or linked current footer, keeps substring matching for historical aliases, and
rejects empty markers. Replaces the attribution-footer entry of 2026-08-16.
